...
enum EntityType { PLATFORM, PLAYER, ENEMY, COLLECTIBLE, HEART };
enum AIType { WALKER_LR, WALKER_UD, GUARD, OBSTACLE, BOUNCER };
...
private:
    ...
    bool m_collided_map_top = false;
    bool m_collided_map_bottom = false;
    bool m_collided_map_left = false;
    bool m_collided_map_right = false;
...
public:
    ...
    void move_left() { m_movement.x = -0.75f; face_left(); }
    void move_right() { m_movement.x = 0.75f;  face_right(); }
    void move_up() { m_movement.y = 0.75f;  face_up(); }
    void move_down() { m_movement.y = -0.75f; face_down(); }
